...
// Dirubah 
const pengirim = "My Boyfriend.";
const idtelegram = "qblueside"
const texttele = "hayo mau ngetik apa?"
const pesan = `ur name is like a melody that always lingers in my mind.`;

if (pengirim) {
  nama.innerHTML = pengirim;
  nama1.innerHTML = pengirim;
} else {
  nama.innerHTML = "My boyfriend.";
  nama1.innerHTML = "My boyfriend.";
}
...
hilang1.addEventListener('click', () => {
  ...
  Swal.fire({
    ...
    imageHeight: 120,
    title: 'kiw, siapa nih nama lengkap nya cantik?',
    ...
    let txt = `hello ${result.value.login}, such a beautiful name. ${pesan}`;
...
hilang2.addEventListener('click', () => {
    window.open(
        `https://t.me/${idtelegram}?text=${encodeURIComponent(texttele)}`,
        '_blank')
})
